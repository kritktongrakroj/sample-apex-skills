...
description: ""
custom_edit_url: https://github.com/aws-samples/sample-apex-skills/blob/main/skills/eks-ingress-migration/references/migration-risk.md
format: md
---

:::info[Source]
This page is generated from [skills/eks-ingress-migration/references/migration-risk.md](https://github.com/aws-samples/sample-apex-skills/blob/main/skills/eks-ingress-migration/references/migration-risk.md). Edit the source, not this page.
:::
...
2. Map each to Gateway API equivalent or workaround:
   ...
   - nginx basic auth → ALB + Cognito/OIDC (on the Gateway path: `ListenerRuleConfiguration` with `authenticateOIDCConfig`, attached via `spec.infrastructure.parametersRef` — **not** an `alb.ingress.kubernetes.io/*` annotation, which LBC ignores on a Gateway) — **browser callers only**; non-interactive clients break (see `alb-migration.md` Authentication)
