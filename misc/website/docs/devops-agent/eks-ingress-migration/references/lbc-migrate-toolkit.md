---
title: "`lbc-migrate` — AWS Load Balancer Controller (LBC) Ingress → Gateway API Automation Toolkit"
description: ""
custom_edit_url: https://github.com/aws-samples/sample-apex-skills/blob/main/devops-agent/eks-ingress-migration/references/lbc-migrate-toolkit.md
format: md
---

:::info[Source]
This page is generated from [devops-agent/eks-ingress-migration/references/lbc-migrate-toolkit.md](https://github.com/aws-samples/sample-apex-skills/blob/main/devops-agent/eks-ingress-migration/references/lbc-migrate-toolkit.md). Edit the source, not this page.
:::
...
> **⚠️ Scope — this skill assesses, it does not perform the migration, and in this runtime it cannot run `lbc-migrate` at all.** The whole toolkit is an **owner / local-machine** workflow: `lbc-migrate` is a CLI that must be built from source (`git clone` + `make`), it writes YAML to a local directory, and `--console` serves a local web UI. This runtime has **no shell, no CLI execution and no file writes**, so **every** `lbc-migrate` step below — including Translate and the console diff — is something the **cluster owner runs on their own machine** and reports back; the assessment *documents* the workflow and *reads the results through the Kubernetes API*. Where a step's outcome is visible in-cluster, the reachable equivalent is a **read-only API GET** (e.g. read the `gateway.k8s.aws/dry-run-plan` annotation off the `Gateway` object rather than running the console). **Everything that changes the cluster or AWS is additionally an OPERATOR / DevOps action the assessor documents but MUST NOT execute** — installing the Gateway API / LBC Gateway CRDs (`kubectl apply`), enabling the `IngressPlanAnnotation` feature gate (`helm upgrade`), **applying the generated dry-run manifests (Step 2 — creates live `Gateway` objects in the cluster, even though no ALB is provisioned)**, applying Gateway manifests with `--dry-run=false` or `kubectl annotate` (Step 3 — creates real ALBs), and `kubectl delete ingress` (Step 6). Present these as steps for the cluster owner to run under their change-management process, never as commands the assessment runs. (Matches the sibling references: *all assessment checks are read-only.*)
...
1. **Translate** *(OWNER action — runs on the owner's machine; not runnable in this runtime)* — `lbc-migrate --from-cluster --namespaces <ns> --output-dir ./gw/`. Reads the cluster (`get`/`list`) only and writes YAML locally, so it is **non-mutating** — but it still needs a shell and local filesystem, which this runtime does not have. Ask the owner to run it and share the output. Rollback: delete the generated files.
2. **Dry-run preview (recommended)** *(generating the manifests locally and inspecting a plan are read-only; enabling the feature gate and applying the dry-run manifests are OPERATOR actions)* — verify the generated Gateway will produce the *same* ALB config before creating anything:
   ...
   - Compare: `lbc-migrate --console` (field-by-field diff) is an **OWNER action** — it is a local web UI and cannot run here. The **reachable equivalent for this runtime** is a read-only Kubernetes API **GET** of the `Gateway` object and its `gateway.k8s.aws/dry-run-plan` annotation, which the assessment reads directly.
...
> **Shared IngressGroup warning.** If the Ingress being migrated participates in a shared ALB (`alb.ingress.kubernetes.io/group.name`), the ALB is **co-owned by every Ingress in that group**. Deleting one member at cleanup does **not** delete the ALB, and conversely the ALB does not go away until the **last** member is removed — so a per-Ingress migration of a grouped estate leaves the original ALB (and its cost) standing until the whole group has moved. Migrate a shared group as a **unit**, and state the group membership in the report so the cleanup step is not mistaken for a per-route operation.
